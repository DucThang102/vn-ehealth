<template>
  <div v-if="xetnghiemList">
    <table class="table table-bordered">
      <tr>
        <th style="width: 5%;" class="text-center">STT</th>
        <th style="width: 15%;" class="text-center">Thao tác</th>
        <th style="width: 60%;" class="text-center">Loại xét nghiệm</th>
        <th style="width: 20%;" class="text-center">Ngày thực hiện</th>
      </tr>
      <tr v-for="(xetnghiem, i) in xetnghiemList" :key="xetnghiem.id">
        <td class="text-center">{{ i + 1 }}</td>
        <td class="text-center">
          <a href="#" v-on:click="viewXetnghiem(xetnghiem)">
            <i class="fas fa-fw fa-binoculars"></i> Xem
          </a>
        </td>
        <td class="text-center">{{ xetnghiem.emrDmLoaiXetNghiem.ten }}</td>
        <td class="text-center">{{ xetnghiem.ngaythuchien }}</td>
      </tr>
      <tr v-if="xetnghiemList.length==0">
        <td colspan="5">Không có dữ liệu</td>
      </tr>
    </table>

    <div class="modal fade" id="xetnghiemModal">
      <div class="modal-dialog modal-lg">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title">
              <strong>Thông tin xét nghiệm</strong>
            </h5>
            <button type="button" class="close" data-dismiss="modal">
              <span aria-hidden="true">&times;</span>
            </button>
          </div>
          <div v-if="xetnghiem" class="modal-body">
            <TomTat :hsbaId="hsbaId" title="PHIẾU XÉT NGHIỆM"></TomTat>
            <hr />
            <font>
              <div class="row">
                <div class="col-12">
                  <b>Xét nghiệm: {{xetnghiem.emrDmLoaiXetNghiem.ten}}</b>
                  <br />
                  <span>- Ngày yêu cầu: {{ formatDateTime(xetnghiem.ngayyeucau) }}</span>
                  <br />
                  <span>- Bác sĩ yêu cầu: {{ attr(xetnghiem, 'bacsiyeucau.ten') }}</span>
                  <br />
                  <span>- Ngày thực hiện: {{ formatDateTime(xetnghiem.ngaythuchien) }}</span>
                  <br />
                  <span>- Bác sĩ thực hiện: {{ attr(xetnghiem, 'bacsixetnghiem.ten') }}</span>
                  <br />
                  <span>- Nhận xét chung: {{ xetnghiem.nhanxet }}</span>
                  <br />
                </div>
              </div>
              <hr />
              <div v-if="xetnghiem.emrXetNghiemDichVus.length > 0">
                <b>Kết quả xét nghiệm :</b>
                <table class="table table-bordered mt-3">
                  <tr>
                    <th style="width:5%" class="text-center">STT</th>
                    <th style="width:25%" class="text-center">Xét nghiệm</th>
                    <th style="width:20%" class="text-center">Kết quả đo</th>
                    <th style="width:20%" class="text-center">Đơn vị</th>
                    <th style="width:30%" class="text-center">Chỉ số bình thường</th>
                  </tr>
                  <template v-for="(xndv, i) in xetnghiem.emrXetNghiemDichVus">
                    <template v-if="xndv.emrXetNghiemKetQuas.length==1">
                      <tr :key="i">
                        <td class="text-center">{{ i + 1 }}</td>
                        <td>{{ xndv.emrDmXetNghiem.ten }}</td>
                        <td class="text-center">{{ xndv.emrXetNghiemKetQuas[0].giatrido }}</td>
                        <td class="text-center">{{ xndv.emrDmXetNghiem.donvi }}</td>
                        <td class="text-center">
                          Nam: {{xndv.emrDmXetNghiem.chisobt_nam}} /
                          Nữ: {{xndv.emrDmXetNghiem.chisobt_nu}}
                        </td>
                      </tr>
                    </template>
                    <template v-if="xndv.emrXetNghiemKetQuas.length>1">
                      <tr :key="i">
                        <td :rowspan="1+xndv.emrXetNghiemKetQuas.length">{{ i + 1 }}</td>
                        <th colspan="4">{{ xndv.emrDmXetNghiem.ten }}</th>
                      </tr>
                      <tr v-for="xnkq in xndv.emrXetNghiemKetQuas" :key="xnkq.id">
                        <td>{{ xnkq.emrDmChiSoXetNghiem.ten }}</td>
                        <td class="text-center">{{ xnkq.giatrido }}</td>
                        <td class="text-center">{{ xnkq.emrDmChiSoXetNghiem.donvi }}</td>
                        <td class="text-center">
                          Nam: {{xnkq.emrDmChiSoXetNghiem.chisobt_nam}} /
                          Nữ: {{xnkq.emrDmChiSoXetNghiem.chisobt_nu}}
                        </td>
                      </tr>
                    </template>
                  </template>
                </table>
              </div>

              <div v-if="xetnghiem.emrFileDinhKemXetNghiems.length > 0">
                <hr />
                <b>Danh sách file đính kèm:</b>
                <table class="table table-bordered mt-3">
                  <tr>
                    <th style="width:10%" class="text-center">STT</th>
                    <th style="width:90%" class="text-center">Tên file</th>
                  </tr>
                  <tr v-for="(file, i) in xetnghiem.emrFileDinhKemXetNghiems" :key="file.id">
                    <td>{{ i + 1 }}</td>
                    <td>
                      <a :href="file.url">{{ file.ten }}</a>
                    </td>
                  </tr>
                </table>
              </div>
            </font>
          </div>
          <div class="modal-footer">
            <button type="button" class="btn btn-secondary" data-dismiss="modal">Đóng lại</button>
          </div>
        </div>
      </div>
    </div>
  </div>
</template>

<script>
import TomTat from "@/components/hsba/view_detail/TomTat.vue";

export default {
  components: {
    TomTat
  },

  props: ["hsbaId"],

  data() {
    return {
      xetnghiemList: null,
      xetnghiem: null
    };
  },

  methods: {
    viewXetnghiem: function(xetnghiem) {
      this.xetnghiem = xetnghiem;
      $("#xetnghiemModal").modal();
    }
  },

  created: async function() {
    this.xetnghiemList = await this.get("/api/xetnghiem/get_ds_xetnghiem", {
      hsba_id: this.hsbaId
    });
    this.xetnghiemList.forEach(xn => {
      xn.emrXetNghiemDichVus = xn.emrXetNghiemDichVus.filter(
        xndv => xndv.emrXetNghiemKetQuas.length > 0
      );
    });
  }
};
</script>